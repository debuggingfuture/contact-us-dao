...
const { PROPOSAL_CONTENT_URI, PROPOSAL_DESCRIPTION, proposalsFile } = require("../app-config");
const fs = require("fs");
...
async function propose(args, functionToCall, proposalDescription) {
  ...
  console.log(`Proposing ${functionToCall} on ${stamp.address} with ${args}`);
...
propose([PROPOSAL_CONTENT_URI], "stampWithContentUri", PROPOSAL_DESCRIPTION)
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error)
    process.exit(1)
